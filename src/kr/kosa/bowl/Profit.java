...
public class Profit implements Serializable {
	...
	private Profit() {
		...
		List<Receipt> savedReceipts = fileIO.loadFile();
		this.receiptList = (savedReceipts != null) ? savedReceipts : new ArrayList<>();
	}
	...
	public int getTotalGameProfit() {
		return receiptList.stream().mapToInt(
				receipt -> receipt.getGameFee() * receipt.getLane().getHeadCnt() * receipt.getLane().getGameCnt())
				.sum();
	}

	/** 전체 기간 신발 대여비 매출 조회 */
	public int getTotalShoesProfit() {
		return receiptList.stream().mapToInt(receipt -> receipt.getShoesFee() * receipt.getLane().getShoesCnt()).sum();
	}

	/** 전체 기간 간식 매출 조회 */
	public int getTotalSnackProfit() {
		return receiptList.stream().mapToInt(receipt -> {
			int snackTotal = 0;
	...
	public int getMonthlyGameProfit(String inputMonth) {
		int month = Integer.parseInt(inputMonth);
		return receiptList.stream().filter(receipt -> getMonth(receipt.getLane().getSelectedAt()) == month).mapToInt(
				receipt -> receipt.getGameFee() * receipt.getLane().getHeadCnt() * receipt.getLane().getGameCnt())
				.sum();
	}
	...
	public int getMonthlyShoesProfit(String inputMonth) {
		int month = Integer.parseInt(inputMonth);
		return receiptList.stream().filter(receipt -> getMonth(receipt.getLane().getSelectedAt()) == month)
				.mapToInt(receipt -> receipt.getShoesFee() * receipt.getLane().getShoesCnt()).sum();
	}
