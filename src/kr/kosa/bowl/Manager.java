...
public class Manager {
	...
	private ProfitFileHandler ps; // 추가
	...
	/** 생성자 */
	public Manager(Profit profit) { // 추가
		this.profit = profit;
		this.ps = new ProfitFileHandler();
	}
...
//		Profit.getInstance().showReceiptList();
	...
	private void getProfitAll() {
		...
		System.out.println(profit.getReceiptList().toString());
	...
	private void getProfitByMonth() {
		...
		do {
			...
			if (inputMonth.isBlank()) {
				...
				System.out.println(
						inputMonth + "월의 총 매출액은 : " + Profit.getInstance().getMonthlyProfit(inputMonth) + "원입니다.");
			}
